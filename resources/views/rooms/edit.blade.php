<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-text-primary leading-tight">{{ __('Edit Room') }}</h2>
    </x-slot>
            ...
            <form method="POST" action="{{ route('rooms.update', $room) }}">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="floor_id" name="floor_id" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">
                            @foreach ($floors as $floor)
                                <option value="{{ $floor->id }}" @selected(old('floor_id', $room->floor_id) == $floor->id)>{{ $floor->name }} ({{ $floor->location->name }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">{{ old('description', $room->description) }}</textarea>
