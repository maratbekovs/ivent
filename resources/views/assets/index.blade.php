                                ...
                                <tbody class="bg-white divide-y divide-neutral-200">
                                    @foreach ($assets as $asset)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500">
                                                {{ $asset->room->name ?? __('Warehouse') }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500">
                                                {{ $asset->currentUser->name ?? '-' }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                <a href="{{ route('assets.show', $asset) }}" class="text-secondary-600 hover:text-secondary-900 mr-3">
                                                    {{ __('View') }}
                                                </a>
                                                <a href="{{ route('assets.edit', $asset) }}" class="text-primary-600 hover:text-primary-900 mr-3">
                                                    {{ __('Edit') }}
                                                </a>
                                                <form action="{{ route('assets.destroy', $asset) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-rose-600 hover:text-rose-900" onclick="return confirm('{{ __('Are you sure you want to delete this asset?') }}')">
                                                        {{ __('Delete') }}
                                                    </button>
                                                </form>
                                            </td>
