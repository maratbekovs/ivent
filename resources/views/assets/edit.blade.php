            ...
            <form method="POST" action="{{ route('assets.update', $asset) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="asset_status_id" name="asset_status_id" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">
                            @foreach ($statuses as $status)
                                <option value="{{ $status->id }}" @selected(old('asset_status_id', $asset->asset_status_id) == $status->id)>{{ $status->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="room_id" name="room_id" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">
                            ...
                            @foreach ($rooms as $room)
                                <option value="{{ $room->id }}" @selected(old('room_id', $asset->room_id) == $room->id)>{{ $room->name }} ({{ $room->floor->location->name ?? '' }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="current_user_id" name="current_user_id" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected(old('current_user_id', $asset->current_user_id) == $user->id)>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="notes" name="notes" rows="4" class="block mt-1 w-full border-gray-300 focus:border-primary focus:ring-primary rounded-md shadow-sm">{{ old('notes', $asset->notes) }}</textarea>
